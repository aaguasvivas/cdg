...
void ControlDependence::toDot(std::string name) const {
  ...
  std::string errInfo;
//  std::error_code ec;
//  raw_fd_ostream out(name.c_str(), ec, 0);

  // check for errors
  if (!out.is_open()) {
    errs() <<  "[Warning] Error opening output file: " << name.c_str() << '\n';
    return;
  }
...
void ControlDependence::insertDotNode(std::ofstream &out, BasicBlock *node) const {
...
void ControlDependence::insertDotEdge(std::ofstream &out, BasicBlock *A, BasicBlock *B) const {
